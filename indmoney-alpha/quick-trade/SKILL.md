...
name: quick-trade
description: |
  Find Indian stocks with high-confluence setups for 1–4 week near-term trades using INDmoney
  live data. Uses multi-timeframe analysis (3-screen method), Fibonacci entry zones, RSI divergence
  detection, ATR-based stop-losses, and R:R validation (minimum 2:1) before recommending any trade.
  Returns top 3 picks with entry price, stop-loss, target, position size, and trailing stop plan.

  Use when the user asks: "quick trade", "short term trade", "what to buy this week",
  "near term profit", "intraday or swing trade", "trade setup", "stocks to buy now",
  "which stock to trade", "momentum stock to buy", or any request for a short-term stock
  trade idea with entry and exit levels for Indian equities.

  Requires INDmoney MCP server connected. Uses: lookup_ind_keys, get_indian_stocks_details
  (with analyst segment), get_indian_stocks_ohlc (1week/1day intervals).
---
