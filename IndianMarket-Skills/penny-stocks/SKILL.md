...
name: penny-stocks
description: |
  Scan Indian penny stocks (priced under ₹100) with real business fundamentals using INDmoney
  live data. Applies multi-timeframe trend analysis, RSI divergence, Fibonacci 52W position,
  ATR-based stops, and 3:1 R:R filter to classify each stock as BUY / SPECULATIVE / AVOID.
  Returns position sizing constraints and stop-loss levels for each pick.

  Use when the user asks: "penny stocks", "low price stocks to invest", "stocks under 100 rupees",
  "cheap stocks with potential", "small price big return stocks", "best penny stocks India",
  "penny stock recommendation", "which penny stock to buy", or any question about low-price
  Indian stocks with investment potential on NSE/BSE.

  Requires INDmoney MCP server connected. Uses: lookup_ind_keys, get_indian_stocks_details
  (with analyst and news segments), get_indian_stocks_ohlc (1week interval, 1y lookback).
---
...
Stop = current_price - ATR × 2.5   (wider for penny stock volatility)
```
...
Never put more than 10% of total portfolio across all penny stocks combined.
...
End with: "Penny stocks carry amplified risk. Max 10% total portfolio exposure. Always use a stop-loss."
